...
- **部分改訂: 2026-09-19 / プランナー** —— **起動時フェードイン(黒 → 透明、既定2.0秒)を反映**(ディレクター指示。developer 実装済み。共通仕様 §2.0)。
  **この画面の Prefab・要素・レイアウトは一切変更していない。**フェードは `Main.unity` の常設オブジェクト `BootFadeLayer` がこの画面の**上に**重なるだけ。
  **起動直後はフェードが最前面で `blocksRaycasts = true` のため、`PlayButton_Time90` / `SettingsButton` はタップされない**(§4.2 に追記)。**既存の 0.3秒の入力遅延とは独立した別の仕組み。**
...
> **★2026-09-19: アプリ起動時に限り、上の処理と並行して `BootFadeLayer` の起動時フェードイン(既定2.0秒)がこの画面の上に重なっている**(共通仕様 §2.0)。
> **この画面側の処理は何も変わらない**(フェードの完了を待つ・フェードに合わせて演出を遅らせる、といった連携はしていない)。
> したがって **`FirstTimeHint` の表示や `PlayButton_Time90` の脈動演出(§4.5)は、黒に隠れている間も進んでいる。**
> 初回起動時に「案内が出た瞬間」を見せたい場合は連携が必要になるが、**現時点では不要と判断している**(§7-15)。
...
**★2026-09-19: 起動時フェードイン(共通仕様 §2.0)との関係 —— 2つは独立した別の仕組み**

| | 起動時フェードイン(2026-09-19 新設) | この画面の入力遅延 0.3秒 |
|---|---|---|
| 担当 | `Main.unity` の常設 `BootFadeLayer`(`BootFadeController`) | `TitleScreenController` |
| 効く範囲 | **画面全体**(最前面の不透明 `Image` が `blocksRaycasts = true` でタップを受け止める) | `PlayButton_Time90` / `SettingsButton` の `interactable` |
| 効く場面 | **アプリ起動時の1回だけ** | **この画面に入るたび**(設定の「もどる」/ 結果・ポーズの「やめる」を含む) |
| 長さ | `GameBalanceSettings.BootFadeInSeconds`(既定 **2.0秒**) | `GameBalanceSettings.TitleInputDelaySeconds`(**0.3秒**) |

- **優先順位や排他制御は設けない。長い方が自然に効く。**起動時は **2.0秒のフェードが支配的**で、その間に 0.3秒の遅延も終わっている。
- **0.3秒の遅延は廃止しない。** フェードは起動時の1回しか再生されないため、**②の「結果画面の『やめる』からの残留タップ」を防げるのは 0.3秒の遅延だけ**である。
- **ただし ①(起動直後の連打)はフェードによって実質的に二重に守られる形になった。**フェードの長さを `0` にした場合は 0.3秒の遅延だけが残る。
...
| **15** ★ | **起動時フェードイン中に、この画面の初回案内(`FirstTimeHint` + 脈動演出)が黒に隠れたまま進んでいてよいか**(§4.1 / §4.5) | **初回起動時のみ。**フェード 2.0秒のうちに脈動が1往復以上進む。**プランナー推奨: このままでよい**(脈動はループ演出で「始まりの瞬間」を見せる必要がなく、連携を入れると仕組みが増える)。**気になる場合のみ、フェード完了後に演出を開始する連携を追加依頼する** | ディレクター(実機確認) |
| **16** ★ | **起動時フェードインの長さ 2.0秒 の適否**(共通仕様 §14-62)。**0.3秒の入力遅延は廃止しない**(§4.2) | フェードを `0` にすると 0.3秒の遅延だけが残る | ディレクター(実機確認) |
