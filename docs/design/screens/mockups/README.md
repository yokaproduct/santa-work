...
> **★2026-09-17 更新**: `Screen_ModeSelect`(しごとを えらぶ)は **`Screen_Title` に統合**されました(`02_Screen_Title.md` 第3版)。
> **`Screen_ModeSelect.png` は不要です。**ゲーム開始ボタン・記録・告知・設定ボタンは、すべて `Screen_Title.png` に描いてください。
...
| 描いたもの | ファイル名 | 対応する仕様書 |
|---|---|---|
| タイトル(ホーム。★2026-09-17 しごとを えらぶ を統合) | `Screen_Title.png` | `02_Screen_Title.md`(第3版) |
| ~~しごとを えらぶ~~ | ~~`Screen_ModeSelect.png`~~ | **★2026-09-17 廃止**(`99_廃止_Screen_ModeSelect.md` は廃止記録。描かなくてよい) |
...
- 1枚にまとめると、地域ごとに「色が変わる」ができません(★2026-09-10 以降、押せる判定は1枚絵の色で行っています。★2026-09-17 地図上の数字の表示は廃止しました)
- **★現状: 地図は `WorldMap.png`(6地域を色で塗り分けた1枚絵)で実装済みです。**別ファイルが必要になるのは、地域ごとに色を変える演出を入れる場合です(決定ログ §4-5)
