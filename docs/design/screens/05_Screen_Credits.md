...
| おんがく | ~~ロイヤリティフリー音源(BGM 3曲)~~ **ロイヤリティフリー音源、またはオリジナル(自作)曲**(BGM 3曲。企画書 §8.4)。**既存クリスマス曲の録音物は使わない**。★2026-09-16: 仮BGM `bgm.ogg`(`bgm_title`)はディレクター申告により自作と確認 |
| こうかおん | SE ~~17点~~ ★18点(2026-09-15 `se_finish` 追加)。★2026-09-16: 仮SE4点(`merry.wav` / `finish.wav` / `yes.wav` / `no.wav`)はディレクター申告により自作と確認 |
| イラスト | 有料アセット / 生成AI / 自作の別を明記。★2026-09-16: 仮置き画像25点と世界地図 `WorldMap.png` はディレクター申告により自作と確認(クレジットに載せるかは決定ログ §4-9 残-3) |
...
| 6 | 開発者名 / スタジオ名の表記 | ストア掲載名と揃える必要がある。**★2026-09-16: 自作素材(現在の `credits.csv` は author「自作(ディレクター制作)」)の作者表記もここで決める。リリース前に確定**(決定ログ §4-9 残-2) | ディレクター |
| **7** ★ | **自作の素材をクレジットに載せるか**(2026-09-16 追加)。音声5点は `credits.csv` に記録済み。**自作画像(25点 + `WorldMap.png`)は未記録** | 載せる場合はカテゴリ「イラスト」「こっき・ちず」の行が増える | ディレクター / 取りまとめ役 |
